...
export default class Widget {
	...
	set_title(max_chars) {
		...
		if (this.icon) {
			...
			this.title_field[0].innerHTML = `${icon} <span class="ellipsis tc-override" title="${title}">${title}</span>`;
		} else {
			this.title_field[0].innerHTML = `<span class="ellipsis tc-override" title="${title}">${title}</span>`;
			if (max_chars) {
